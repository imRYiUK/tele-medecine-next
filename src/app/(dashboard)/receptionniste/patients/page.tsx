...
export default function ReceptionnistePatientsPage() {
  ...
  function NouveauPatientDialog({ onPatientCreated }: { onPatientCreated: () => void }) {
    // Reset du formulaire à la fermeture
    const handleDialogOpenChangeLocal = (isOpen: boolean) => {
      setOpen(isOpen);
      if (!isOpen) {
        setForm(initialForm);
        setError(null);
    ...
    return (
      <Dialog open={open} onOpenChange={handleDialogOpenChangeLocal}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                ...
                <Input id="nom" name="nom" placeholder="Nom" value={form.nom} onChange={handleChange} required ref={initialRef} />
              </div>
              <div className="flex flex-col gap-1">
                <label htmlFor="prenom" className="text-sm font-medium text-gray-700">Prénom</label>
                <Input id="prenom" name="prenom" placeholder="Prénom" value={form.prenom} onChange={handleChange} required />
              </div>
              <div className="flex flex-col gap-1">
                <label htmlFor="dateNaissance" className="text-sm font-medium text-gray-700">Date de naissance</label>
                <Input id="dateNaissance" name="dateNaissance" type="date" value={form.dateNaissance} onChange={handleChange} required />
              </div>
              <div className="flex flex-col gap-1">
                <label htmlFor="genre" className="text-sm font-medium text-gray-700">Genre</label>
                <select id="genre" name="genre" value={form.genre} onChange={handleChange} className="border rounded px-2 py-2 focus:ring-2 focus:ring-emerald-500">
              ...
              <div className="flex flex-col gap-1 md:col-span-2">
                ...
                <Input id="adresse" name="adresse" placeholder="Adresse complète" value={form.adresse} onChange={handleChange} required />
              </div>
              <div className="flex flex-col gap-1">
                <label htmlFor="telephone" className="text-sm font-medium text-gray-700">Téléphone</label>
                <Input id="telephone" name="telephone" placeholder="Numéro de téléphone" value={form.telephone} onChange={handleChange} required />
              </div>
              <div className="flex flex-col gap-1">
                <label htmlFor="email" className="text-sm font-medium text-gray-700">Email</label>
                <Input id="email" name="email" type="email" placeholder="Adresse email" value={form.email} onChange={handleChange} required />
              </div>
              <div className="flex flex-col gap-1">
                <label htmlFor="groupeSanguin" className="text-sm font-medium text-gray-700">Groupe sanguin</label>
                <select id="groupeSanguin" name="groupeSanguin" value={form.groupeSanguin} onChange={handleChange} className="border rounded px-2 py-2 focus:ring-2 focus:ring-emerald-500">
              ...
              <div className="flex flex-col gap-1">
                ...
                <select id="etatDossier" name="dossierMedical.etatDossier" value={form.dossierMedical.etatDossier} onChange={handleChange} className="border rounded px-2 py-2 focus:ring-2 focus:ring-emerald-500">
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex gap-2">
          <NouveauPatientDialog onPatientCreated={fetchPatients} />
